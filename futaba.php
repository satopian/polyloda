...
// 拒否設定--------------------------------------------------------------------
$badstring = array("dummy_string","dummy_string2","wwww","ｗｗ","無料動画","無料画像","友達募集");      // 拒絶する文字列
...
define("EN_DELTIME", 1);                                // 消える予告時間(使用しない：0、使用する：1)
...
function form(&$dat,$resno,$admin="",$name_flag=FALSE){
  ...
  if(!$resno || RES_IMG){
    ...
    if(!$resno) {
      $dat .= ' [<label><input type="checkbox" name="chkname" value="on">名前欄なし</label>] ';
      $dat .= ' [<label><input type="checkbox" name="chkid" value="on">強制ID表示</label>] ';
...
<blockquote>もう、書き込めませんよ。<br><br>停止しました。。。<br>スレッドストッパー。。。(￣ー￣)ﾆﾔﾘｯ</blockquote></td></tr></table>
__STOP__;
    }
    // hiro 追加 ここまで
...
//    list(,,,$chkemail,,,$chkurl,,,,,,$ltime,) = explode(",", rtrim($line[$lineindex[$resto]-1]));
    ...
    // hiro 追加 2005.03.24
    if(stristr($chkurl, '_tstop_')) { error("スレッドストッパー。。。(￣ー￣)ﾆﾔﾘｯ", $dest); }
    // hiro 追加 ここまで
...
function CleanStr($str){
  ...
  if($admin!=ADMIN_PASS){//管理者はタグ可能
    if(PHP_VERSION_ID >= 50400){$str = htmlspecialchars($str, ENT_COMPAT | ENT_HTML401, 'SJIS');} else {$str = htmlspecialchars($str);}     //タグっ禁止 ◆Yakuba PHP5.4.0以降対応
...
//      if(empty($line[$i])) { continue; }
  ...
  for($j = 0; $j < count($line); $j++){
    ...
    if(PHP_VERSION_ID >= 50400){$com = htmlspecialchars($com, ENT_COMPAT | ENT_HTML401, 'SJIS');} else {$com = htmlspecialchars($com);}     // ◆Yakuba PHP5.4.0以降対応
